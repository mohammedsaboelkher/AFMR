...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Audio Watermarking Benchmarking with Square Attack"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the directory containing your watermarked .wav files",
    )
    parser.add_argument(
        "--secret_message",
        type=str,
        required=True,
        help="The universal bitstring message embedded in all audio files (e.g., 10101010)",
    )
    parser.add_argument(
        "--blackbox_folder",
        type=str,
    ...
    parser.add_argument(
        "--encode", action="store_true", help="Run the encoding process before decoding"
    )
    parser.add_argument(
        "--length", type=int, default=5 * 16000, help="Length of the audio samples"
    )
    parser.add_argument("--gpu", type=int, default=0, help="GPU device index to use")
    parser.add_argument(
        "--save_pert", action="store_true", help="If set, saves the perturbed waveform"
    )
    parser.add_argument(
        "--query_budget", type=int, default=10000, help="Query budget for the attack"
    )
    parser.add_argument(
        "--eps",
        type=float,
        default=0.05,
        help="Epsilon boundary constraint for the attack",
    )
    parser.add_argument(
        "--p",
        type=float,
        default=0.05,
        help="Initial relative size of the coordinate selection patch",
    )
    parser.add_argument(
        "--tau",
        type=float,
        default=0.0,
        help="Threshold bit-accuracy target for the detector (stop condition)",
    )
    parser.add_argument(
        "--snr",
        type=list,
        default=[0, 2.5, 5, 7.5, 10, 12.5, 15, 17.5, 20, 22.5, 25, 27.5, 30],
        help="Signal-to-noise ratio boundaries",
    )
    parser.add_argument(
        "--norm",
        type=str,
        default="linf",
        choices=["linf", "l2"],
        help="Norm constraint for the attack",
    )
    parser.add_argument(
        "--attack_bitstring",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Spectrogram components to attack",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="",
        choices=["audioseal", "wavmark", "timbre"],
        required=True,
        help="Watermarking model architecture to evaluate",
    )
...
def square_attack_linf(
    ...
    for i_iter in progress_bar:
        ...
        for i_img in range(x_best_curr.shape[0]):
            ...
            x_curr_window = x_curr[
                i_img, :, center_h : center_h + s, center_w : center_w + s
            ]
            x_best_curr_window = x_best_curr[
                i_img, :, center_h : center_h + s, center_w : center_w + s
            ]
            while (
                np.sum(
                ...
                deltas[i_img, :, center_h : center_h + s, center_w : center_w + s] = (
                    np.random.choice([-eps, eps], size=[c, 1, 1])
                )
        ...
        progress_bar.set_description(
            f"iter: {i_iter + 1}, acc: {acc:.2f}, max_pert_best: {curr_norms_image_best:.2f}"
        )
        if acc <= args.tau:
            break
...
def square_attack_l2(
    ...
    for counter in range(h // s):
        ...
        for counter2 in range(w // s):
            delta_init[:, :, center_h : center_h + s, center_w : center_w + s] += (
                ...
                * np.random.choice([-1, 1], size=[x.shape[0], c, 1, 1])
            )
            center_w += s
        center_h += s
    x_best = np.clip(
        x
        + delta_init
        / np.sqrt(np.sum(delta_init**2, axis=(1, 2, 3), keepdims=True))
        * eps,
        min_val,
        max_val,
    )
    ...
    for i_iter in progress_bar:
        ...
        new_deltas_mask_2[
            :, :, center_h_2 : center_h_2 + s2, center_w_2 : center_w_2 + s2
        ] = 1.0
        curr_norms_window = np.sqrt(
            np.sum(
                ((x_best_curr - x_curr) * new_deltas_mask) ** 2,
                axis=(2, 3),
                keepdims=True,
            )
        )
        ...
        new_deltas = np.ones([x_curr.shape[0], c, s, s]) * meta_pseudo_gaussian_pert(
            s
        ).reshape([1, 1, s, s])
        new_deltas *= np.random.choice([-1, 1], size=[x_curr.shape[0], c, 1, 1])
        old_deltas = delta_curr[
            :, :, center_h : center_h + s, center_w : center_w + s
        ] / (1e-10 + curr_norms_window)
        new_deltas += old_deltas
        new_deltas = (
            new_deltas
            / np.sqrt(np.sum(new_deltas**2, axis=(2, 3), keepdims=True))
            * (np.maximum(eps**2 - curr_norms_image**2, 0) / c + norms_windows**2)
            ** 0.5
        )
        delta_curr[:, :, center_h_2 : center_h_2 + s2, center_w_2 : center_w_2 + s2] = (
            0.0
        )
        delta_curr[:, :, center_h : center_h + s, center_w : center_w + s] = (
            new_deltas + 0
        )
        x_new = (
            x_curr
            + delta_curr
            / np.sqrt(np.sum(delta_curr**2, axis=(1, 2, 3), keepdims=True))
            * eps
        )
...
class WatermarkDetectorWrapper(PyTorchClassifier):
    ...
    def get_detection_result_audioseal(self, spectrogram: np.ndarray) -> np.ndarray:
        ...
        if self.on_bitstring:
            if msg_decoded is None:
                return np.array([0.0])
            bitacc = 1 - torch.sum(torch.abs(self.message - msg_decoded)) / len(
                self.message
            )
    ...
    def get_detection_result_wavmark(self, spectrogram: np.ndarray) -> np.ndarray:
        ...
        spectrogram_tensor = torch.tensor(spectrogram).to(device=self._device)
        signal = (
            self.transform.spectrogram2signal(spectrogram_tensor)
            .squeeze(0)
            .detach()
            .cpu()
        )
        ...
        bit_acc = (
            1
            - torch.sum(torch.abs(payload_tensor - self.message))
            / self.message.shape[0]
        )
    ...
    def get_detection_result_timbre(
        self, spectrogram: np.ndarray, batch_size: int = 1
    ) -> np.ndarray:
...
class signal22spectrogram:
    ...
    def signal2spectrogram(self, signal: torch.Tensor) -> torch.Tensor:
        ...
        if self.attack_type == "amplitude":
            ...
            return torch.cat([spectro_amplitude, spectro_phase], dim=0)[
                ..., self.lf : self.hf, :
            ]
...
def decode_audio_files_perturb_blackbox(
    ...
    watermarked_files = [
        f for f in os.listdir(args.input_dir) if f.lower().endswith(".wav")
    ]
    ...
    original_payload = torch.tensor(
        list(map(int, args.secret_message)), dtype=torch.int
    )
    ...
    for watermarked_file in progress_bar:
        ...
        waveform, sample_rate = torchaudio.load(
            os.path.join(args.input_dir, watermarked_file)
        )
        ...
        detector = WatermarkDetectorWrapper(
            ...
            original_payload,
            "single-tailed",
            args.attack_bitstring,
            transform,
            args.tau,
            model_type=args.model,
            input_size=transform.attack_shape,
            device=device,
        )

        watermarked_spectrogram = (
            transform.signal2spectrogram(waveform).unsqueeze(0).detach().cpu().numpy()
        )
        n_queries, adv_spectrogram = attack(
            detector, watermarked_spectrogram, args.eps, args.query_budget, args.p, args
        )

        adv_signal = transform.spectrogram2signal(
            torch.tensor(adv_spectrogram).to(device)
        )
        ...
        with open(filename, "a") as log:
            log.write(
                f"{watermarked_file}, {int(n_queries.item())}, {acc}, {snr}, {visqol_score}\n"
            )

        torchaudio.save(
            os.path.join(args.blackbox_folder, f"adv_{file_idx}.wav"),
            adv_signal.detach().cpu(),
            sample_rate,
        )
...
def main() -> None:
    ...
    if args.model == "audioseal":
        ...
        model_config = yaml.load(
            open("timbre/config/model.yaml", "r"), Loader=yaml.FullLoader
        )
        ...
        checkpoint = torch.load(
            "timbre/results/ckpt/pth/compressed_none-conv2_ep_20_2023-02-14_02_24_57.pth.tar"
        )
